...
class KioskSetupActivity : AppCompatActivity() {
    ...
    private lateinit var appOneSpinner: Spinner
    private lateinit var appTwoSpinner: Spinner
    ...
    private lateinit var appAdapter: ArrayAdapter<AppChoice>
    ...
    private fun buildUi() {
        ...
        content.addView(TextView(this).apply {
            text = "Setup Awal Hope Tablet"
            textSize = 24f
            setTypeface(typeface, android.graphics.Typeface.BOLD)
        })
        content.addView(TextView(this).apply {
            text = "Atur mode penggunaan tablet. Pilihan ini tersimpan di dalam aplikasi Hope."
            textSize = 14f
        ...
        content.addView(Button(this).apply {
            text = "Buka Info Aplikasi Hope (izin terbatas Infinix/Android)"
        ...
        content.addView(Button(this).apply {
            text = "Matikan optimasi baterai untuk Hope"
        ...
        content.addView(TextView(this).apply {
            text = "Catatan: HOPe sekarang dapat dipilih sebagai launcher/Home default. Setelah simpan setup, buka Default Home Settings dan pilih HOPe. Auto-start tetap dipertahankan sebagai cadangan. Untuk kiosk penuh yang menahan Home/Recent/Back, factory reset tablet lalu jalankan ADB:\n${kioskManager.getDeviceOwnerCommand()}"
            textSize = 13f
            setPadding(0, dp(8), 0, dp(16))
        })
        ...
        modeGroup.addView(radio("Kunci hanya aplikasi Hope", MODE_ID_HOPE_ONLY))
        modeGroup.addView(radio("Launcher HOPe + aplikasi tambahan yang diizinkan", MODE_ID_HOPE_PLUS_ALLOWED))
        ...
        appAdapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, choices)
        appOneSpinner = Spinner(this).apply { adapter = appAdapter }
        appTwoSpinner = Spinner(this).apply { adapter = appAdapter }
        allowedAppsContainer.addView(TextView(this).apply { text = "Aplikasi tambahan awal yang diizinkan. Aplikasi lain bisa ditambah dari Pengaturan Launcher HOPe." })
        allowedAppsContainer.addView(appOneSpinner)
        allowedAppsContainer.addView(appTwoSpinner)
    ...
    private fun refreshPermissionStatus() {
        ...
        homeStatus.text = if (kioskManager.isHopeDefaultHome()) { "✓ Launcher/Home default: HOPe" } else { "✕ Launcher/Home default: belum HOPe" }
    ...
    private fun updateAllowedAppsVisibility() {
        ...
        allowedAppsContainer.visibility = if (selectedMode() == KioskPrefs.MODE_HOPE_PLUS_ALLOWED) View.VISIBLE else View.GONE
    }

    private fun selectedMode(): String {
        return when (modeGroup.checkedRadioButtonId) {
            MODE_ID_HOPE_ONLY -> KioskPrefs.MODE_HOPE_ONLY
            MODE_ID_HOPE_PLUS_ALLOWED -> KioskPrefs.MODE_HOPE_PLUS_ALLOWED
    ...
    private fun saveAndContinue() {
        ...
        val allowedPackages = if (mode == KioskPrefs.MODE_HOPE_PLUS_ALLOWED) {
            listOfNotNull(
                (appOneSpinner.selectedItem as? AppChoice)?.packageName,
                (appTwoSpinner.selectedItem as? AppChoice)?.packageName,
            ).filter { it.isNotBlank() && it != packageName }.distinct().take(2)
        } else {
            emptyList()
        }

        prefs.saveSetup(mode, pin.ifBlank { "0000" }, allowedPackages)
        startActivity(Intent(this, HopeHomeActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP))
        finish()
    }
    ...
    private fun loadLaunchableApps(): List<AppChoice> {
        ...
        return listOf(AppChoice("Tidak ada", "")) + apps
    }
    ...
    companion object {
        ...
        private const val MODE_ID_HOPE_ONLY = 1002
        private const val MODE_ID_HOPE_PLUS_ALLOWED = 1003
    }
}
