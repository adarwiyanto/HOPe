package id.my.hopenoodles.hopepos.kiosk
...
class KioskManager(private val context: Context) {
    ...
    private val adminComponent = ComponentName(context, HopeDeviceAdminReceiver::class.java)
    ...
    fun openAutoStartSettings(activity: Activity) {
        ...
        Toast.makeText(
            activity,
            "Halaman Auto-start Infinix tidak bisa dibuka otomatis. Silakan aktifkan manual dari Info Aplikasi HOPe > Battery/Auto-start.",
            Toast.LENGTH_LONG,
        ).show()
    ...
    fun openDeviceAdminSettings(activity: Activity) {
        val intent = Intent(DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN).apply {
            ...
            putExtra(
                ...
                "Aktifkan hanya bila diminta. Untuk kiosk penuh, Hope harus menjadi Device Owner lewat ADB/factory reset.",
            )
        }
        activity.startActivity(intent)
    }
    ...
    fun applyHomeLauncherAsDeviceOwner() {
        ...
        dpm.addPersistentPreferredActivity(
            adminComponent,
            filter,
            ComponentName(context.packageName, "${context.packageName}.ui.HopeHomeActivity"),
        )
    }
    ...
    fun hideSystemUi(activity: Activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            activity.window.insetsController?.let { controller ->
                ...
                // bisa dipakai untuk kembali ke HOPe Launcher saat Lock Task aktif.
    ...
    fun getDeviceOwnerCommand(): String {
        return "adb shell dpm set-device-owner ${context.packageName}/.kiosk.HopeDeviceAdminReceiver"
    }
