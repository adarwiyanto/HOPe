...
require_admin();
require_menu_access('admin.users');
ensure_owner_role();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    if ($action === 'delete') {
      ...
      if ($id > 0) {
        ...
        if ($target && (int)$target['id'] !== (int)($me['id'] ?? 0)) {
          if (!current_user_is_owner() && normalize_role_key((string)($target['role'] ?? '')) === 'owner') {
    ...
    if ($action === 'update_role') {
      ...
      if (($role['role_key'] ?? '') === 'owner') throw new Exception('Assign role owner dibatasi permanen.');
      if ($id > 0 && $id !== (int)($me['id'] ?? 0)) {
        $stmt = db()->prepare("UPDATE users SET role=?, role_id=? WHERE id=?");
        $stmt->execute([$role['role_key'], (int)$role['id'], $id]);
        redirect(base_url('admin/users.php'));
      }
    }
    ...
    if ($action === 'invite') {
      ...
      $role = role_by_id($roleId);
      if (!$role) throw new Exception('Role tidak valid.');
      if (($role['role_key'] ?? '') === 'owner') throw new Exception('Undangan role owner dinonaktifkan demi keamanan.');
      ...
      $stmt->execute([$email, $role['role_key'], $tokenHash, $expiresAt]);

      if (!send_invite_email($email, $token, $role['role_key'])) {
...
$users = db()->query("SELECT u.id, u.username, u.name, u.role, u.role_id, u.created_at, r.role_name, r.role_key
  FROM users u
      ...
      <div class="grid cols-2">
        <div class="card">
          ...
          <?php if (current_user_is_owner()): ?>
            <form method="post">
              ...
              <div class="row"><label>Role</label><select name="role_id"><?php foreach ($roles as $role): if (($role['role_key'] ?? '') === 'owner') continue; ?><option value="<?php echo e((string)$role['id']); ?>"><?php echo e($role['role_name']); ?></option><?php endforeach; ?></select></div>
              <button class="btn" type="submit">Kirim Undangan</button>
              <p><small>Link undangan berlaku 2 hari.</small></p>
            </form>
          <?php else: ?><p><small>Hanya owner yang bisa mengundang user.</small></p><?php endif; ?>
        </div>
        ...
        <div class="card">
          ...
          <table class="table">
            ...
            <tbody>
              <?php foreach ($users as $u): $roleKey = normalize_role_key((string)($u['role_key'] ?? $u['role'] ?? '')); $roleLabel = $u['role_name'] ?? ucfirst($roleKey); ?>
                <tr>
                  <td><?php echo e($u['username']); ?></td><td><?php echo e($u['name']); ?></td><td><span class="badge"><?php echo e($roleLabel); ?></span></td><td><?php echo e($u['created_at']); ?></td><td>
                    <?php if (current_user_is_owner() && (int)$u['id'] !== (int)($me['id'] ?? 0)): ?>
                      <form method="post" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo e(csrf_token()); ?>"><input type="hidden" name="action" value="update_role"><input type="hidden" name="id" value="<?php echo e($u['id']); ?>"><select name="role_id"><?php foreach ($roles as $role): if (($role['role_key'] ?? '') === 'owner') continue; ?><option value="<?php echo e((string)$role['id']); ?>" <?php echo ((int)$u['role_id']===(int)$role['id']) ? 'selected' : ''; ?>><?php echo e($role['role_name']); ?></option><?php endforeach; ?></select><button class="btn" type="submit">Simpan</button></form>
                      <form method="post" data-confirm="Hapus user ini?" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo e(csrf_token()); ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo e($u['id']); ?>"><button class="btn" type="submit">Hapus</button></form>
                    <?php elseif (!current_user_is_owner() && (int)$u['id'] !== (int)($me['id'] ?? 0) && $roleKey !== 'owner'): ?>
                      <form method="post" data-confirm="Hapus user ini?" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo e(csrf_token()); ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo e($u['id']); ?>"><button class="btn" type="submit">Hapus</button></form>
                    <?php endif; ?>
                  </td>
                </tr>
        ...
        <div class="card">
          ...
          <?php if (current_user_is_owner()): ?>
            <form method="post"><input type="hidden" name="_csrf" value="<?php echo e(csrf_token()); ?>"><input type="hidden" name="action" value="save_email_settings">
              ...
              <div class="row"><label>SMTP Security</label><select name="smtp_secure"><option value="ssl" <?php echo ($mailCfg['secure'] ?? '') === 'ssl' ? 'selected' : ''; ?>>SSL (465)</option><option value="tls" <?php echo ($mailCfg['secure'] ?? '') === 'tls' ? 'selected' : ''; ?>>TLS (STARTTLS)</option><option value="none" <?php echo ($mailCfg['secure'] ?? '') === 'none' ? 'selected' : ''; ?>>None</option></select></div>
          ...
          <?php else: ?><p><small>Pengaturan email hanya tersedia untuk owner.</small></p><?php endif; ?>
        </div>
      </div>
    </div>
  </div>
